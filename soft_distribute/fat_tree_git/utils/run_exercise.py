...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, **opts):
        ...
        for link in host_links:
            ...
            self.addLink(host_name, sw_name, port2=sw_port, delay=link['delay'], bw=link['bandwidth'], jitter=link['jitter'], loss=link['loss'], max_queue_size=link['max_queue_size'])
            #print (link['delay'], link['bandwidth'], link['jitter'], link['loss'], link['max_queue_size'])
        ...
        for link in switch_links:
            ...
            self.addLink(sw1_name, sw2_name,
                        ...
                        delay=link['delay'], bw=link['bandwidth'], jitter=link['jitter'], loss=link['loss'], max_queue_size=link['max_queue_size'])
            #print (link['delay'], link['bandwidth'], link['jitter'], link['loss'], link['max_queue_size'])
