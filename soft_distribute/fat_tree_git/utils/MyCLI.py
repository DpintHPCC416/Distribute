...
class MyCLI( Cmd ):
    ...
    def do_check(self, line):
        ...
        timeout = None
        if len(args) < 3:
            error('usage: check hi hj packet_num [timeout]\n')
            return
        if len(args) == 4:
            timeout = int(args[3])
        host1 = args[0]
        host2 = args[1]
        total_packets = int(args[2])
        node1 = self.mn.getNodeByName(host1)
        node2 = self.mn.getNodeByName(host2)
        if len(args) == 4:
            timeout = int(args[3])
            node2.popen("python test/recv.py %d" % timeout)
        else:
            node2.popen("python test/recv.py")
        node1.cmd("python test/send.py %s %d" % (node2.IP(), total_packets))

    def do_checkall(self, line):
        total_packets = 100
        args = line.split()
        if len(args) == 1:
            total_packets = int(args[0])
        for host in self.mn.hosts:
            host.popen("python test/recv.py")
        #send one by one
        for src in self.mn.hosts:
            ...
            for dst in self.mn.hosts:
                if dst == src:
                    continue
                output('%s ' % dst.name)
                src.cmd("python test/send.py %s %d" % (dst.IP(), total_packets))
                time.sleep(5)
            output("\n")
        time.sleep(5)
        for host in self.mn.hosts:
            host.cmd('python test/killRecv.py')
